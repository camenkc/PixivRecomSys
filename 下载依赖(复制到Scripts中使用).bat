pip install flask -i https://douban.com/simple
pip install flask_moment -i https://mirrors.aliyun.com/pypi/simple/
pip install scrapy -i https://mirrors.aliyun.com/pypi/simple/
pip install fake_useragent -i https://douban.com/simple
pip install requests -i https://mirrors.aliyun.com/pypi/simple/
pip install beautifulsoup4 -i https://mirrors.aliyun.com/pypi/simple/
pip install lxml -i https://mirrors.aliyun.com/pypi/simple/
pip install pymysql -i https://mirrors.aliyun.com/pypi/simple/
pip install -U requests[socks] -i https://mirrors.aliyun.com/pypi/simple/
pause